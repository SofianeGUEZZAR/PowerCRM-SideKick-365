...
class EntityListProcess extends ProcessButton {
    static id = "entitiesList";
    constructor() {
        super("entitiesList");
        this.process = EntityList;
    }
}

const EntityList = forwardRef<ProcessRef, ProcessProps>(function EntityListProcess(props: ProcessProps, ref) {
    ...
    const entitiesFiltered = useMemo(() => {
        ...
        return entities?.filter(
            (e) => e.name.toLowerCase().includes(filterLower) || e.logicalname.toLowerCase().includes(filterLower)
        );
    }, [entities, filter]);
    ...
    return (
        <Stack spacing={1} height="calc(100% - 20px)" padding="10px" alignItems="center">
            <FilterInput
                fullWidth
                placeholder="Search Name or LogicalName"
                defaultValue={filter}
...
export default EntityListProcess;
