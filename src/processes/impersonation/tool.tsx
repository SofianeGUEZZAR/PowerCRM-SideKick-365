...
import {
    ConvertToActiveUserObject,
    RetrieveActiveUsersWithSecurityRoles
} from "../../utils/hooks/XrmApi/RetrieveActiveUsersWithSecurityRoles";
...
class ImpersonationProcess extends ProcessButton {
    ...
    constructor() {
        super("impersonate");
        this.process = Impersonation;
    }

    onExtensionLoad(snackbarProviderContext: ProviderContext): void {
        MessageManager.sendMessage(MessageType.GETIMPERSONATION).then(async function (
            existingRules: chrome.declarativeNetRequest.Rule[] | null
        ) {
            const url = Xrm.Utility.getGlobalContext().getClientUrl();
            const currentRule = existingRules?.find(
                (r) => r.condition.urlFilter?.includes(url) && !r.condition.urlFilter.includes("RemovedAction")
            );
            ...
            if (currentRule) {
                ...
                if (currentAzureIdOrUserId) {
                    const result = await Xrm.WebApi.online.retrieveMultipleRecords(
                        "systemuser",
                        `?$select=systemuserid,fullname&$filter=(systemuserid eq ${currentAzureIdOrUserId} or azureactivedirectoryobjectid eq ${currentAzureIdOrUserId})&$expand=systemuserroles_association($select=roleid,name,roleidunique),teammembership_association($select=teamid,name)`
                    );
                    if (result.entities.length > 0) {
                        const user: ActiveUser = await ConvertToActiveUserObject(result.entities[0]);

                        snackbarProviderContext.enqueueSnackbar(
                            <>
                                You are impersonating <b>{user.fullname}</b> ({user.systemuserid})
                            </>,
                            {
                                variant: "detailsFile",
                                detailsVariant: "info",
...
const Impersonation = forwardRef<ProcessRef, ProcessProps>(function ImpersonationProcess(props: ProcessProps, ref) {
    ...
    const completion = useMemo(
        () => (totalActiveUsers > 0 ? Math.round((loadedActiveUsers / totalActiveUsers) * 100) : 0),
        [loadedActiveUsers, totalActiveUsers]
    );

    const sendNewUserToBackground = useCallback(
        (newUser: ActiveUser | null) => {
            const data = {
    ...
    useEffect(() => {
        ...
        MessageManager.sendMessage(MessageType.GETIMPERSONATION).then(function (
            ...
            if (currentRule) {
                ...
                if (currentAzureId) {
                    const impersonateUser = activeUsers.find(
                        (u) => u.azureObjectId === currentAzureId || u.systemuserid === currentAzureId
                    );
    ...
    const filteredActiveUsers = useMemo(() => {
        return activeUsers.filter((user) => {
            if (
                securityRoleSelected.filter(
                    (selectedRole) =>
                        user.securityRoles.filter((securityRole) => securityRole.roleid === selectedRole.roleid)
                            .length +
                        (user.teamsRoles?.filter((teamRole) => teamRole.roleid === selectedRole.roleid).length ?? 0)
                ...
                <Box>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center" }}>
                        ...
                        <Box sx={{ minWidth: 35 }}>
                            <Typography
                                variant="body2"
                                sx={{ color: "text.secondary" }}>{`${completion || 0}%`}</Typography>
                        </Box>
                    </Box>
                </Box>
            )}
                ...
                <List sx={{ width: "100%", height: "100%" }}>
                    <MuiVirtuoso
                        ...
                        itemContent={(index, user) => {
                            return <UserItem user={user} userSelected={selectedUser} handleSelect={handleSelect} />;
                        }}
                    />
                </List>
            ) : (
...
const UserItem = React.memo((props: UserItemProps) => {
    ...
    return (
        <ListItem key={user.systemuserid} disablePadding>
                    ...
                    {securityRoles.map((role) => {
                        ...
                        return (
                            <SecurityRoleItem
                                handleSelect={handleSelect}
                                role={role}
                                securityRoleSeclected={securityRoleSeclected}
                            />
                        );
                    })}
                </List>
...
export default ImpersonationProcess;
