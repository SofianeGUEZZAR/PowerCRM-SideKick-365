...
const OptionSetTableProcess = forwardRef<ProcessRef, ProcessProps>(
    function OptionSetTableProcess(props: ProcessProps, ref) {
        ...
        const { entityName: currentEntityName, forceRefresh } =
            useCurrentRecord();
        ...
        const [pickList, isFetchingPickList] = RetrievePicklistValues(
            entityName,
            MSType.Picklist
        );
        const [multiPickList, isFetchingMultiPickList] = RetrievePicklistValues(
            entityName,
            MSType.MultiSelectPicklist
        );
        const [stateList, isFetchingStateList] = RetrievePicklistValues(
            entityName,
            MSType.State
        );
        const [statusList, isFetchingStatusList] = RetrievePicklistValues(
            entityName,
            MSType.Status
        );
        ...
        useEffect(() => {
            ...
            const optionSetTable = Object.entries(
                allFieldOptions
            ).reduce<OptionSetTables>(
                (
                    previousValue: OptionSetTables,
                    [currentLogicalName, currentOptions]
                ) => {
                    return {
                        ...previousValue,
                        [currentOptions.Name]: {
                            ...
                            Fields: [
                                ...(previousValue[currentOptions.Name]
                                    ?.Fields ?? []),
                                currentLogicalName
                            ],
                            DisplayName:
                                currentOptions.DisplayName.UserLocalizedLabel
                                    ?.Label ?? currentOptions.Name,
        ...
        const copyAll = useCallback(() => {
            const tableElements = Array.from(
                listRef.current?.querySelectorAll<HTMLTableElement>(
                    `.${TABLE_CLASS}`
                ) ?? []
            );
            ...
            navigator.clipboard.write([
                new ClipboardItem({
                    "text/html": new Blob(
                        tableElements.flatMap((tableElement) => [
                    ...
                    <List
                        ...
                        }}>
                        {
                            isFetching ? (
                                ...
                                Object.entries(optionSetTable).map(
                                    ([pickListLogicalName, metadata]) => {
                                        const lowerFilter =
                                            filter.toLowerCase();
                                        if (
                                            pickListLogicalName
                                            ...
                                            metadata.DisplayName?.toLowerCase().includes(
                                                lowerFilter
                                            ) ||
...
function OptionSetTable(props: OptionSetTableProps) {
    ...
    const tooltip = useMemo(
        () => (
            <>
                ...
                <Typography>
                    <b>IsCustomOptionSet:</b>{" "}
                    {String(metadata.IsCustomOptionSet)}
                </Typography>
            </>
        ),
        [metadata]
    ...
    const isExistingStateColumn = useMemo(
        () =>
            metadata.Options.some(
                (o) => o.State !== undefined && o.State !== null
            ),
        [metadata.Options]
    );
    const isExistingDefaultStatusColumn = useMemo(
        () =>
            metadata.Options.some(
                (o) => o.DefaultStatus !== undefined && o.DefaultStatus !== null
            ),
        [metadata.Options]
    );
    const isExistingParentValuesColumn = useMemo(
        () =>
            metadata.Options.some(
                (o) => o.ParentValues && o.ParentValues.length > 0
            ),
        [metadata.Options]
    );
    ...
    return (
        <ListItem>
            <TableContainer
                key={`tableContainer${logicalName}`}
                component={Paper}
                elevation={3}>
                        ...
                        {metadata.Options.map((o) => {
                            return (
                                <Tooltip
                                    title={
                                        (o.Color ||
                                            o.Description.UserLocalizedLabel
                                                ?.Label ||
                                            o.ExternalValue) && (
                                            <>
                                                {o.Color && (
