...
import { RetrievePicklistValues, type PickListOption } from "../../utils/hooks/XrmApi/RetrievePicklistValues";
import { MSType } from "../../utils/types/requestsType";

class OptionSetTableProcess extends ProcessButton {
    static id = "optionsettable";
    constructor() {
        super("optionsettable");
        this.process = OptionSetTableMain;
    }
}
...
const OptionSetTableMain = forwardRef<ProcessRef, ProcessProps>(function OptionSetTableProcess(
    props: ProcessProps,
    ref
) {
    ...
    const isFetching = useMemo(
        () => isFetchingPickList || isFetchingMultiPickList || isFetchingStateList || isFetchingStatusList,
        [isFetchingPickList, isFetchingMultiPickList, isFetchingStateList, isFetchingStatusList]
    );
    ...
    useEffect(() => {
        ...
        const optionSetTable = Object.entries(allFieldOptions).reduce<OptionSetTables>(
            (previousValue: OptionSetTables, [currentLogicalName, currentOptions]) => {
                return {
                    ...previousValue,
                    [currentOptions.Name]: {
    ...
    const copyAll = useCallback(() => {
        ...
        navigator.clipboard.write([
            new ClipboardItem({
                "text/html": new Blob(
                    tableElements.flatMap((tableElement) => [tableElement.outerHTML, "<p></p>"]),
                    {
                        type: "text/html"
                    }
                ),
                "text/plain": new Blob(
                    tableElements.flatMap((tableElement) => [tableElement.innerText, "\r\n"]),
                    {
                        type: "text/plain"
                    }
                )
            ...
            <Stack spacing={1} height="calc(100% - 10px)" padding="10px" pr={0} pt={0} alignItems="center">
                <Stack direction="column" width="95%" spacing={0.5}>
                    <Stack direction="row" spacing={1} mt={1} mb={1} alignItems="center">
                        ...
                        <Button variant="contained" onClick={forceRefresh} sx={{ whiteSpace: "nowrap", width: "25%" }}>
                            Refresh
                        </Button>
                    </Stack>
                    <Stack direction="row" spacing={1} mt={1} mb={1} alignItems="center">
                        <FilterInput
                            fullWidth
                            placeholder="Search by name or columns"
                            defaultValue={filter}
                            returnFilterInput={setFilter}
                        />
                        <Button variant="outlined" onClick={copyAll} sx={{ whiteSpace: "nowrap", width: "25%" }}>
                            {tablesCopied ? "Copied!" : "Copy All"}
                        </Button>
                    </Stack>
                </Stack>
...
function OptionSetTable(props: OptionSetTableProps) {
    ...
    const tooltip = useMemo(
        () => (
            <>
                <Typography>
                    <b>DisplayName:</b> {metadata.DisplayName}
                </Typography>
                <Typography>
                    <b>OptionSetType:</b> {metadata.OptionSetType}
                </Typography>
                <Typography>
                    <b>IsCustomOptionSet:</b> {String(metadata.IsCustomOptionSet)}
                </Typography>
            </>
        ),
        [metadata]
    ...
    const isExistingStateColumn = useMemo(
        () => metadata.Options.some((o) => o.State !== undefined && o.State !== null),
        [metadata.Options]
    );
    const isExistingDefaultStatusColumn = useMemo(
        () => metadata.Options.some((o) => o.DefaultStatus !== undefined && o.DefaultStatus !== null),
        [metadata.Options]
    );
    const isExistingParentValuesColumn = useMemo(
        () => metadata.Options.some((o) => o.ParentValues && o.ParentValues.length > 0),
        [metadata.Options]
    );
                ...
                <Table key={`table_${logicalName}`} className={TABLE_CLASS} ref={tableRef} size="small">
                    <TableHead>
                        <TableRow sx={{ display: "none" }}>
                            ...
                            <TableCell>
                                <Typography>Columns involved: {metadata.Fields.join(", ")}</Typography>
                            </TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell>
                            ...
                            {isExistingStateColumn && (
                                <TableCell align="right">
                                    <b>State</b>
                                </TableCell>
                            )}
                            {isExistingDefaultStatusColumn && (
                                <TableCell align="right">
                                    <b>DefaultStatus</b>
                                </TableCell>
                            )}
                            {isExistingParentValuesColumn && (
                                <TableCell align="right">
                                    <b>ParentValues</b>
                                </TableCell>
                            )}
                        ...
                        {metadata.Options.map((o) => {
                            return (
                                <Tooltip
                                                ...
                                                {o.Color && (
                                                    <Typography display="flex" alignItems="center" gap={0.5}>
                                                        ...
                                                        <Box
                                                            ...
                                                            width={13}
                                                            height={13}
                                                            border="1px solid #EEEEEE"></Box>{" "}
                                                        {o.Color}
                                                    </Typography>
                                                )}
                                                {o.Description.UserLocalizedLabel?.Label && (
                                                    <Typography>
                                                        <b>Description:</b> {o.Description.UserLocalizedLabel?.Label}
                                                    </Typography>
                                                )}
                                                {o.ExternalValue && (
                                                    <Typography>
                                                        <b>ExternalValue:</b> {o.ExternalValue}
                                                    </Typography>
                                                )}
                                            </>
                                        )
                                    ...
                                    placement="left">
                                    <TableRow>
                                        <CopyTableCell copyText={o.Label.UserLocalizedLabel?.Label ?? ""}>
                                            {o.Label.UserLocalizedLabel?.Label}
                                        </CopyTableCell>
                                        {isExistingStateColumn && (
                                            <CopyTableCell align="right" copyText={String(o.State)}>
                                                {o.State}
                                            </CopyTableCell>
                                        )}
                                        {isExistingDefaultStatusColumn && (
                                            <CopyTableCell align="right" copyText={String(o.DefaultStatus)}>
                                                {o.DefaultStatus}
                                            </CopyTableCell>
                                        )}
                                        {isExistingParentValuesColumn && (
                                            <CopyTableCell align="right" copyText={JSON.stringify(o.ParentValues)}>
                                                {JSON.stringify(o.ParentValues)}
                                            </CopyTableCell>
                                        )}
                                        <CopyTableCell align="right" copyText={String(o.Value)}>
                                            {o.Value}
                                        </CopyTableCell>
                                    </TableRow>
                                </Tooltip>
...
export default OptionSetTableProcess;
