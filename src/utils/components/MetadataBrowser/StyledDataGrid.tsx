...
const StyledGridOverlay = styled("div")(({ theme }) => ({
    ...
    height: "100%",
    "& .no-rows-primary, .no-results-primary": {
        fill: "#3D4751",
        ...theme.applyStyles("light", {
            fill: "#AEB8C2"
        })
    },
    "& .no-rows-secondary, .no-results-secondary": {
        fill: "#1D2126",
        ...theme.applyStyles("light", {
            fill: "#E8EAED"
        })
...
export function CustomLoadingOverlay(props: any | { whatIsLoading?: string }) {
    return (
        <StyledGridOverlay>
            <CircularProgress />
            <Box sx={{ mt: 2 }}>
                Retrieving {props.whatIsLoading ?? "rows"}…
            </Box>
        </StyledGridOverlay>
    );
}
