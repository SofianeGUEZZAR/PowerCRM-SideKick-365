...
import { type PluginTraceLog } from "./type";
...
const PluginTraceLogsPane = React.memo((props: PluginTraceLogsPaneProps) => {
    ...
    const [firstPluginTraceLogs, isFetchingFirst, refreshFirst]: [
        PluginTraceLog | undefined,
        boolean,
        () => void
    ] = RetrieveFirstRecordInterval(
        "plugintracelog",
        ["plugintracelogid"],
        "",
        "performanceexecutionstarttime desc"
    );

    const {
        pluginTraceLogs,
        isFetchingPluginTraceLogs,
        refreshPluginTraceLogs
    } = useContext(TraceLogsAPIContext);
    ...
    const pluginTraceLogsFiltered = useMemo(
        () =>
            pluginTraceLogs.filter((log) => {
                const textFilterResult =
                    log.primaryentity.includes(filterEntity.toLowerCase()) &&
                    log.messagename
                        .toLowerCase()
                        .includes(filterMessageName.toLowerCase());
                const dateFilterMin =
                    !filterDateMin ||
                    dayjs(log.performanceexecutionstarttime) >= filterDateMin;
                const dateFilterMax =
                    !filterDateMax ||
                    dayjs(log.performanceexecutionstarttime) <= filterDateMax;
                if (errorOnly) {
                    return (
    ...
    const dateTimeFormat = useMemo(
        () => getCurrentDynamics365DateTimeFormat(),
        []
    );
            ...
            <Stack direction="column" width="100%" height="100%">
                <Stack direction="column" padding={1} spacing={0.5}>
                    <Stack direction="row" spacing={0.5}>
                        ...
                        <FilterInput
                            fullWidth
                            returnFilterInput={setFilterMessageName}
                            placeholder="Message Name"
                        />
                        <Tooltip
                            title={
                                <Typography variant="body2">
                                    Display only logs with errors
                                </Typography>
                            }
                            placement="top-end"
                            disableInteractive
                    ...
                    <ButtonLinearProgress
                        loading={isFetchingPluginTraceLogs}
                        onClick={refreshPluginTraceLogs}
                        variant="contained">
                        Refresh ({decount})
                    </ButtonLinearProgress>
                </Stack>
                <PluginTraceLogsList
                    pluginTraceLogs={pluginTraceLogsFiltered}
                    isFetching={isFetchingPluginTraceLogs || isFetchingFirst}
                />
            </Stack>
            <TraceLogDialog />
        </ThemeProvider>
